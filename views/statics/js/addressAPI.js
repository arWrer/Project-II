...
function execDaumPostcode1() {
    ...
    new daum.Postcode({
        oncomplete: function(data1) {
            ...
            var extraAddr = ''; // 참고항목 변수
            //사용자가 선택한 주소 타입에 따라 해당 주소 값을 가져온다.
            if (data1.userSelectedType === 'R') { // 사용자가 도로명 주소를 선택했을 경우
                addr = data1.roadAddress;
            } else { // 사용자가 지번 주소를 선택했을 경우(J)
                addr = data1.jibunAddress;
            }
            // 사용자가 선택한 주소가 도로명 타입일때 참고항목을 조합한다.
            if(data1.userSelectedType === 'R'){
                // 법정동명이 있을 경우 추가한다. (법정리는 제외)
                // 법정동의 경우 마지막 문자가 "동/로/가"로 끝난다.
                if(data1.bname !== '' && /[동|로|가]$/g.test(data1.bname)){
                    extraAddr += data1.bname;
                }
                // 건물명이 있고, 공동주택일 경우 추가한다.
                if(data1.buildingName !== '' && data1.apartment === 'Y'){
                    extraAddr += (extraAddr !== '' ? ', ' + data1.buildingName : data1.buildingName);
                }
                // 표시할 참고항목이 있을 경우, 괄호까지 추가한 최종 문자열을 만든다.
                if(extraAddr !== ''){
                    extraAddr = ' (' + extraAddr + ')';
            ...
            // 우편번호와 주소 정보를 해당 필드에 넣는다.
            document.getElementById('postcode1').value = data1.zonecode;
